...
export class OperacionService {
  ...
  listarPorEntidad(
    ...
    const params = new HttpParams()
      .set("page", page.toString())
      .set("size", size.toString());
    return this.http.get<PageResponse<Operacion>>(
      `/operaciones/entidad/${idEntidad}`,
      { params },
    );
  }
  ...
  listarPorRangoFechas(
    ...
    const params = new HttpParams()
      ...
      .set("size", size.toString());
    return this.http.get<PageResponse<Operacion>>(
      `/operaciones/rango-fechas/${fechaInicio}/${fechaFin}`,
      { params },
    );
  }
  ...
  buscar(
    ...
    if (filtro.tipoOperacion)
      params = params.set("tipoOperacion", filtro.tipoOperacion);
    if (filtro.idEntidad)
      params = params.set("idEntidad", filtro.idEntidad.toString());
    if (filtro.usuarioRegistro)
      params = params.set("usuarioRegistro", filtro.usuarioRegistro);
    if (filtro.estadoOperacion)
      params = params.set("estadoOperacion", filtro.estadoOperacion);
    if (filtro.fechaInicio)
      params = params.set("fechaInicio", filtro.fechaInicio);
    if (filtro.fechaFin) params = params.set("fechaFin", filtro.fechaFin);

    return this.http.get<PageResponse<Operacion>>("/operaciones/buscar", {
      params,
    });
  }
}
