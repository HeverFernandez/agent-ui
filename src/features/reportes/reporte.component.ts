...
@Component({
  ...
  standalone: true,
  imports: [
    CommonModule, MatCardModule, MatFormFieldModule, MatInputModule, MatSelectModule,
    MatButtonModule, MatIconModule, MatProgressSpinnerModule, MatDatepickerModule,
    MatNativeDateModule, MatTableModule, MatPaginatorModule, NgxChartsModule,
...
export class ReporteComponent implements OnInit {
  ...
  displayedColumns: string[] = ['idOperacion', 'tipoOperacion', 'entidad', 'montoOperacion', 'estadoOperacion', 'fechaOperacion'];
  ...
  loadDatos(): void {
    ...
    forkJoin({
      operaciones: this.operacionService.listar(0, 1000).pipe(catchError(() => of(null))),
      saldos: this.saldoService.listar(0, 1000).pipe(catchError(() => of(null))),
    }).subscribe({
  ...
  buildCharts(ops: Operacion[]): void {
    ...
    this.chartOperacionesPorTipo.set(Object.entries(porTipo).map(([name, value]) => ({ name, value })));
    this.chartOperacionesPorEstado.set(Object.entries(porEstado).map(([name, value]) => ({ name, value })));
    this.chartMontosPorEntidad.set(Object.entries(montosPorEntidad).map(([name, value]) => ({ name, value })));
  }
  ...
  exportarPDF(): void {
    ...
    doc.setFontSize(10);
    doc.text(`Fecha de generación: ${new Date().toLocaleString('es-ES')}`, 14, 28);
    if (this.fechaInicio() && this.fechaFin()) {
    ...
    autoTable(doc, { head, body, startY: 40, theme: 'striped', styles: { fontSize: 8 } });
  ...
  getTipoLabel(tipo: string): string {
    const labels: Record<string, string> = { RETIRO: 'Retiro', DEPOSITO: 'Depósito', PAGO_SERVICIO: 'Pago' };
